...
const rules: ConversionRule[] = [
  ...
  {
    pattern: new RegExp(`第([${KANJI_NUMS}]+)(?!取得者|債務者|者|方|般)`, 'g'),
    replace: (_, n) => `第${convertString(n)}`
  },
  {
    pattern: new RegExp(`前([${KANJI_NUMS}]+)(?!方|般)`, 'g'),
    replace: (_, n) => `前${convertString(n)}`
  },
  {
    pattern: new RegExp(`の(?!一部)([${KANJI_NUMS}]+)(?!方|般)`, 'g'),
    replace: (_, n) => `の${convertString(n)}`
  },
